...
Memory entry ที่ดึงมาจากเซสชันก่อนหน้าจะถูกห่อด้วย tag `<untrusted_memory>` เพื่อให้โมเดลถือว่าเป็นข้อมูลจากผู้ใช้ ไม่ใช่คำสั่งที่เชื่อถือได้ ซึ่งป้องกันการโจมตีแบบ memory poisoning นอกจากนี้ยังมีการ validate ตอนเขียนที่จะปฏิเสธ entry ที่มี XML control tag
...
ระบบ nudge จะทำงานทุก ๆ ไม่กี่ iteration ระหว่างงานที่ยาวเพื่อเตือนให้ agent บันทึกข้อเท็จจริงใหม่ก่อนจบเซสชัน ตั้งค่าด้วย `nudge_interval` ใน `config.yaml` (0 = ปิด)
...
สร้างได้โดยตรงด้วย tool `write_skill` หรือเขียนไฟล์ `SKILL.md` เองก็ได้
